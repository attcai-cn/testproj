...
const addItem = () => {
  formRef.value.validate((valid) => {
    if (valid) {
      request.post('/user/add', tableData.form).then(res => {
        if (res.code === '200') {
...
const updateItem = () => {
  formRef.value.validate((valid) => {
    if (valid) {
      request.put('/user/update', tableData.form).then(res => {
        if (res.code === '200') {
...
const deleteBatch = () => {
  ElMessageBox.confirm('确认删除？删除后无法恢复。',{
    ...
    request.delete('/user/deleteBatch',{ data: tableData.rows}).then(res => {
      if(res.code === '200') {
...
const exportData = () =>{
  ...
  let expUrl = `http://localhost:8080/user/export?name=${tableData.name === null ? '':tableData.name}`
